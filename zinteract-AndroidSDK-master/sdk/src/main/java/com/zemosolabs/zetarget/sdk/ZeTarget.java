    ...
    public class ZeTarget {
        ...
        static JSONObject showIn_App_Text(){
            DbHelper dbHelper = DbHelper.getDatabaseHelper(context);
            JSONObject FetchedData =  dbHelper.getin_app_text(DeviceDetails.getLocale());
            if(FetchedData == null || FetchedData.length() == 0  ){
                if(ZeTarget.isDebuggingOn()){
                    Log.d(TAG,"No Fetched Data Found");
                }

                return null;

            }
            Log.d(TAG,"json="+FetchedData.toString());
            return FetchedData;
        }
        ...
        private static  void fetchIN_App_text() {
            if (ZeTarget.isDebuggingOn()) {
                Log.d(TAG, "httpWorker is now making server request to fetch In_app_text");
            }
            boolean fetchSuccess = false;
            try {
                JSONObject postParams = new JSONObject();
                postParams.put("lastCampaignSynchedTime", CommonUtils.replaceWithJSONNull(getLastCampaignSyncTime()));
                String response = HttpHelper.doPost(Constants.Z_IN_APP_TEXT, postParams);
                if (response != null) {
                    final JSONObject jsonResponse = new JSONObject(response);
                    fetchSuccess = true;
                    logWorker.post(new Runnable() {
                ...
                if (ZeTarget.isDebuggingOn()) {
                    Log.e(TAG, "Exception in fetchIn_App_Text:", e);
                }
            }
        }
       private static void addFetchedText(JSONObject json){
        int addCount;
            if(!json.has("in_app_text")){
                Log.d(TAG,"there is no changed text");
            }
            try{
                JSONArray in_app_text = json.getJSONArray("changed_text");
                if(in_app_text !=null)
                    Log.d(TAG,"in_app_text=" + in_app_text);
                    ...
                    // Just log any other exception so things don't crash on upload
                    if (ZeTarget.isDebuggingOn()) {
                        Log.e(TAG, "Exception in addFetchedText:", e);
                    }
                }
        }
        ...
        private static void addPromotions(JSONObject json) {
            ...
            if (!json.has("promotions")) {
               Log.d(TAG,"promotions are not there");}
                try {
                    JSONArray promotions = json.getJSONArray("promotions");
                    if (promotions != null)
                        Log.d(TAG, "promotions=" + promotions);
...
//Temperorily using showing the campaign on MainScreen when no screenId is available in the JSON.
        ...
        private static void updateDataStore(JSONObject newDataStore){
            ...
            try {
                ...
                JSONArray inapptexts = newDataStore.getJSONArray("inapptext");
                for(int i=0;i<inapptexts.length();i++){
                    JSONObject inapptext = inapptexts.getJSONObject(i);
                    String locale = inapptext.get("locale").toString();
                    JSONArray changedTexts = inapptext.getJSONArray("changed_text");
                    for( int j=0;j<changedTexts.length();j++){
                        JSONObject changedText = changedTexts.getJSONObject(j);
                        String key = changedText.get("key").toString();
                        String value = changedText.get("value").toString();
                        dbHelper.addin_app_Text(value,locale);
        ...
        /**
    u         * Method to log Purchase Attempted event
         */
        ...
        public static Context  doPQR(Activity act, Context ctx)  {
