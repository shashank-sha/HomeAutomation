...
class DbHelper extends SQLiteOpenHelper {
    ...
    private static final String IN_APP_TABLE_NAME = Constants.Z_DB_IN_APP_TABLE_NAME;
    ...
    private static final String CREATE_IN_APP_TABLE = "CREATE TABLE IF NOT EXISTS "+ Constants.Z_DB_IN_APP_TABLE_NAME + " ("
            + Constants.Z_DB_IN_APP_CHANGED_TEXT_FIELD_NAME + "changed_text TEXT," + Constants.Z_DB_IN_APP_CHANGED_LOCALE_FIELD_NAME
            + "TEXT);";
    private File file;
    ...
    public void onCreate(SQLiteDatabase db) {
        ...
        db.execSQL(CREATE_IN_APP_TABLE);
    }
    ...
    public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
        ...
        db.execSQL("DROP TABLE IF EXISTS " + IN_APP_TABLE_NAME);
        db.execSQL(CREATE_EVENTS_TABLE);
        db.execSQL(CREATE_PROMOTIONS_TABLE);
        db.execSQL(CREATE_USERPROPERTIES_TABLE);
        db.execSQL(CREATE_SCREEN_FIX_TABLE);
    ...
    synchronized long addin_app_Text(String changed_text,String locale){
      //  locale =DeviceDetails.getLocale();
        long result =-1,result2=-1;
        try{
            SQLiteDatabase db =getWritableDatabase();
            ContentValues contentValues = new ContentValues();
            ContentValues contentValues1 = new ContentValues();
            contentValues.put(Constants.Z_DB_IN_APP_CHANGED_TEXT_FIELD_NAME,changed_text);
           // contentValues.put("changed_text",changed_text);
            contentValues1.put(Constants.Z_DB_IN_APP_CHANGED_LOCALE_FIELD_NAME,locale);
           // contentValues.put("locale",locale);


            result =db.insertWithOnConflict(Constants.Z_DB_IN_APP_TABLE_NAME,null,contentValues,1);
            result2 =db.insertWithOnConflict(Constants.Z_DB_IN_APP_TABLE_NAME,null,contentValues1,2);
            if (result == -1 || result2== -1) {
                if(ZeTarget.isDebuggingOn()){
                    Log.w(TAG, "Insert failed in changed_text");
                }
            }

        }catch (SQLiteException e) {
            if(ZeTarget.isDebuggingOn()){
                Log.e(TAG, "addChanged_Text failed", e);
            }
            // Not much we can do, just start fresh
            delete();
        } finally {
    ...
    synchronized JSONObject getin_app_text(String locale){
       locale = DeviceDetails.getLocale();
        JSONObject changed_text = new JSONObject();
        Cursor cursor = null;
        try {
            SQLiteDatabase db = getReadableDatabase();
            cursor = db.query(Constants.Z_DB_IN_APP_TABLE_NAME, null, " locale = ?", new String[]{ locale}, null,
                    null, Constants.Z_DB_IN_APP_ID_FIELD_NAME + " DESC", null);
            ...
            while (cursor.moveToNext()) {
                ...
                changed_text.put(key, value);
            }

        } catch (SQLiteException e) {
            if(ZeTarget.isDebuggingOn()){
                Log.e(TAG, "getChanged_text failed", e);
            }
        } catch (Exception e){
            if(ZeTarget.isDebuggingOn()){
                Log.e(TAG, " getChanged_text failed", e);
